...
/// Accumulates lists of objects for one voxel until there are enough
/// to send off.
///
/// Private class of CollisionAggregator. A collision aggregator maintains
/// a sparse hashtable of voxels (i.e. it "creates" only voxels in which it
/// places objects that it is aggregating).
///
/// \see CollisionAggregator
class voxelAggregator {
private:
  //Accumulates objects for the current message
  growableBufferT<CollideObjRec> obj; ///< list of objects in this voxel
  CollideLoc3d destination;  ///< destination voxel ID (3 int coords)
  collideMgr *mgr;           ///< back-pointer to the collide manager

public:
  /// Constructor.
  voxelAggregator(const CollideLoc3d &dest, ///< destination voxel ID (3 int coords)
                  collideMgr *mgr_ ///< back-pointer to the collide manager
                  )
    : destination(dest),mgr(mgr_) {}
  
  /// Add this object to the local list
  inline void add(const CollideObjRec &o) {
    obj.push_back(o);
  }
  
  /// Send off all accumulated objects.
  /// If the local list is not empty, get the raw data from the list,
  /// and ask the collide manager to send a "voxel message" to the 
  /// actual destination (a voxel chare).
  void send(void);
};
				    ...
				    const bbox3d *boxes, const int *prio)
{
  ...
  for (int i=0;i<n;i++) {
    ...
    iSeg1d sy(gridMap.world2grid(1, bbox.axis(1)));
    iSeg1d sz(gridMap.world2grid(2, bbox.axis(2)));
