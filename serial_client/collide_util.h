...
struct CollideObjID {
  ...
  CollideObjID(int chunk_, ///< element (chunk) that created the object
               int number_, ///< index of the object on the element 
               int prio_, ///< priority (collision family) of the object
               int pe_ ///< processor on which the element (chunk) exists
               ) 
...
/// This encapsulates an object ID and the actual box3d
struct CollideObjRec {
  CollideObjID id;
  bbox3d box;
  CollideObjRec(const CollideObjID &id_,
                const bbox3d &box_)
    : id(id_), box(box_) {}
...
class CollideGrid3d {
  ...
  vector3d origin;   ///< origin of the grid (lower-left-bottom corner)
  vector3d scales;   ///< scaling factors in the 3 directions
  vector3d sizes;    ///< grid cell size in the 3 directions
...
public:
  ...
  /// Map world->grid for a coordinate on the specified direction (axis).
  real world2grid(int axis,real x) const {
    return (x-origin[axis])*scales[axis];
  }

  /// Map world->grid a segment along the specified direction (axis).
  ...
  /// Map grid->world a coordinate on the specified direction (axis).
  ...
  /// Map grid->world a segment along the specified direction (axis).
